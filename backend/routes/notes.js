...
router.post(
  ...
  (req, res) => {
    try {
      ...
      console.log(upload);
      console.log("====================================");
      res.json({ file_name: req.file.path });
    } catch (error) {
      console.log("====================================");
      console.log(error);
      console.log("====================================");
...
router.put("/:id/buy", async (req, res) => {
  try {
    ...
    if (!note.buy.includes(req.body.userId)) {
      await note.updateOne({
        $push: { buy: req.body.userId },
      });
      res.status(200).json("The note has been selled");
    } else {
      res
        .status(200)
        .json("You already has been buy this note ");
    }
  } catch (err) {
    res.status(500).json(err);
  }
...
router.get("/:id", async (req, res) => {
  try {
    const note = await Note.findById(req.params.id).limit(
      10
    );
...
router.get("/timeline/:userId", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(userNotes.concat(...followingsNotes));
  } catch (err) {
    res.status(500).json(err);
  }
});
